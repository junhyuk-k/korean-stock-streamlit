...
def load_auto_news():
    rss_urls = {
        "국내 경제": "https://news.google.com/rss/search?q=한국+주식+경제&hl=ko&gl=KR&ceid=KR:ko",
        "반도체": "https://news.google.com/rss/search?q=반도체+산업&hl=ko&gl=KR&ceid=KR:ko",
        "원전": "https://news.google.com/rss/search?q=원전+산업&hl=ko&gl=KR&ceid=KR:ko",
        "방산": "https://news.google.com/rss/search?q=방산+수주&hl=ko&gl=KR&ceid=KR:ko",
        "미국 시장": "https://news.google.com/rss/search?q=US+stock+market&hl=en-US&gl=US&ceid=US:en"
    }
    ...
    for category, rss_url in rss_urls.items():
        ...
        for entry in feed.entries[:10]:
            ...
            if entry.get("source"):
                source_value = entry.source.get("title", "")
...
with tab5:
    ...
    if auto_news_df.empty:
        ...
        st.write(f"현재 수집 뉴스: **{len(auto_news_df)}건**")

        selected_news_category = st.selectbox(
            "뉴스 카테고리",
            ["전체"] + sorted(
                auto_news_df["카테고리"].dropna().unique().tolist()
            ),
            key="auto_news_category"
        )

        if selected_news_category == "전체":
            filtered_auto_news_df = auto_news_df.copy()
        else:
            filtered_auto_news_df = auto_news_df[
                auto_news_df["카테고리"] == selected_news_category
            ].copy()

        st.dataframe(
            filtered_auto_news_df[
                [
                    "카테고리",
                    "제목",
                    "출처",
